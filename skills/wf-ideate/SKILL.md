...
---
# Skill: /wf-ideate — Dual-Engine Product Ideation & Incubation Pipeline
...
python3 .agents/skills/issue-tracker/scripts/report-issue.py \
    ...
    --suggested-action "Execute site setup (site-setup skill) with @designer and @programmer to scaffold MVP" \
...
- **Market Validation (`market-validation` skill)** → Rapidly test buyer willingness to pay via pretotyping, smoke test landing pages, and Mom Test outreach before committing development hours.
- **Site Setup (`site-setup` skill)** → Pre-populates `docs/brand-context.md`, defines design tokens with `@designer`, and scaffolds tech with `@programmer`.
- **Feature Research (`feature-research` skill)** → For modular architectural breakdowns.
- **Planning & Execution (`wf-plan` / `agent-parallelization`)** → Convert into an execution plan and launch parallel subagents.
...
> 3. 🏗️ **Start Direct Setup & Scaffolding:** Trigger `site-setup` to configure brand context, design tokens, and scaffolding.
