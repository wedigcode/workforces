...
- **Autonomous Git Commits & PR-First Review Protocol**: All git operations, commit cadences, and PR creations must adhere to [`git-workflow`](git-workflow.md). Agents commit and push autonomously at deterministic milestones, conducting code reviews within GitHub PRs rather than blocking local execution.
...
- **Scope of Audit Mandate**: The pre-plan codebase audit protocol applies strictly to **major architectural changes, greenfield feature epics, or multi-component refactors**.
- **Fast Path Exemption**: Localized bug fixes, single-file helper edits, targeted troubleshooting, and direct Q&A are strictly exempt from implementation plans and pre-plan audit rituals (see [`lean-execution`](lean-execution.md)).
- **Mandatory Existing Codebase Audit Section**: When an `implementation_plan.md` artifact is required for major features, it MUST include a dedicated section titled `## Existing Codebase Audit Findings`.
- **Pre-Plan Research Enforcement**: Symbol discovery (`code-graph`) and codebase searches (`grep_search`, `list_dir`) MUST execute *before* drafting the implementation plan for new features.
